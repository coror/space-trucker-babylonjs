...
class SpaceTruckerLoadingScreen {
  ...
  constructor(engine) {
    this._totalToLoad = 0.0;
    this._loadingText = 'Loading Space-Truckers: The Video Game...';
    this._currentAmountLoaded = 0.0;
    this._engine = engine;
    this._startScene = createStartScene(engine);
    this._textContainer = AdvancedDynamicTexture.CreateFullscreenUI(
      'loadingUI',
      true,
  ...
  onProgressHandler(evt) {
    ...
    if (this._progressAvailable) {
      this._loadingText =
        ...
        ((this._currentAmountLoaded / this._totalToLoad) * 100).toFixed(2);
    }
  }
}
